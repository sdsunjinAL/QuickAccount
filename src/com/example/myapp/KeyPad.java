...
public class KeyPad extends Activity implements OnClickListener {
	private Button btnDisplay = null;  //顶部展示屏
	...
	private String value = "0"; //获取点击的数字值
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if(this.getIntent().hasExtra("value")) //如果点击按钮，获取点击的按钮值赋给value变量
		{
			value=this.getIntent().getStringExtra("value");
		}
		if(value==null || value.equals("")) //如果未点击，默认为0
		{
			value="0";
			isValueEmpty = true;
		}
		btnDisplay.setText(value); //将value获取的点击的按钮的值显示在按钮屏上
	 ...
	 */
	@Override
	public void onClick(View v) {  //获取屏幕所有的按钮就的点击事件
		// TODO Auto-generated method stub
		switch (v.getId()) {
		case R.id.display:  //屏幕不可点击
			break;
		case R.id.delete:
			if (value.length() > 1) {
				value = value.substring(0, value.length() - 1); //截取value中存储的整个字段内容
			} else {
				value = "0";
			}
			break;
